...
function showTabClickError() {
    ...
    $('.alert-dismissible').delay(2000).fadeOut(600);
}

//When old value selected
...
$(function () {
    ...
    $('.birth_date').on('change', function () {
        let year = $('.birth_date.year').val() || 0,
            month = $('.birth_date.month').val() || 0,
            day = $('.birth_date.date').val() || 0,
            startDate = changeDateFormat(`${day}-${month}-${year}`);
        if (startDate) {
            isBirthDate();
        }
        let today = new Date().toISOString().slice(0, 10)
        const diffInMs = new Date(today) - new Date(startDate)
        const age = Math.round(diffInMs / (1000 * 60 * 60 * 24));
        $('#age').val(age);
    ...
    $("#age").on("change paste keyup", function () {
        ...
        let birthDate = new Date(new Date().getTime() - (ageDays * 24 * 60 * 60 * 1000));
        $('#birth_day_hidden_field').val(birthDate.getDate() + '-' + (birthDate.getMonth() + 1) + '-' + birthDate
            .getFullYear());
    ...
    function getAgeDays(dateVal) {
        let date = changeDateFormat(dateVal),
            birthdayDate = new Date(date),
            todayDate = new Date(),
            diff = new Date(todayDate - birthdayDate),
            day = 1000 * 60 * 60 * 24,
    ...
    //Milking Date Calculate
    $("#days_in_milk").on("change paste keyup", function () {
        let milkDays = $(this).val();
        let calvingDate = new Date(new Date().getTime() - (milkDays * 24 * 60 * 60 * 1000));
        $('#last_calving_date').val(calvingDate.getDate() + '-' + (calvingDate.getMonth() + 1) + '-' +
            calvingDate.getFullYear());
    });
    //Dry Date Calculate
    $("#days_in_dry").on("change paste keyup", function () {
        let dryDays = $(this).val();
        let dryDate = new Date(new Date().getTime() - (dryDays * 24 * 60 * 60 * 1000));
        $('#drying_date').val(dryDate.getDate() + '-' + (dryDate.getMonth() + 1) + '-' + dryDate
            .getFullYear());
    });
    //Pregnant Date Calculate
    $("#pregnancy_days").on("change paste keyup", function () {
        let pdDays = $(this).val();
        let pdDate = new Date(new Date().getTime() - (pdDays * 24 * 60 * 60 * 1000));
        $('#pd_check_date').val(pdDate.getDate() + '-' + (pdDate.getMonth() + 1) + '-' + pdDate
            .getFullYear());
    });
    //AI Date Calculate
    $("#ai_days").on("change paste keyup", function () {
        let aiDays = $(this).val();
        let aiDate = new Date(new Date().getTime() - (aiDays * 24 * 60 * 60 * 1000));
        $('#ai_date').val(aiDate.getDate() + '-' + (aiDate.getMonth() + 1) + '-' + aiDate
            .getFullYear());
    });
});
...
$('.next-button').click(function () {
    damSource = $('input[name="dam_source"]:checked').val(); //farmed or purchased

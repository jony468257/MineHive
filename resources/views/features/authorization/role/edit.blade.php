...
<form action="{{ route('roles.update', $role->id) }}" method="POST" role="form"
    ...
    <div class="row">
        <div class="col-md-3 mb-3">
            <label for="roleNameEdit" class="form-label fw-bold mb-2">{{ __('texts.role_name') }}</label>
            <input type="text" class="form-control" name="name" value="{{ $role->name }}"
                   id="roleNameEdit" placeholder="{{ __('texts.role_name') }}"/>
        </div>
        ...
        <div class="col-md-3">
            ...
            <div class="form-check">
                <input type="checkbox" class="form-check-input"
                       id="check-all-permission-edit" @checked(AuthorizationHelper::roleHasPermissions($role, $permissions))>
                <label class="title-label-select" for="check-all-permission-edit">All</label>
            </div>
        </div>
        <hr class="form-separator">
                    ...
                    <div class="col-md-6 tree-leader">
                        <div class="form-check mt-4 mb-3 tree-item">
                            <input type="checkbox"
                                   ...
                                   id="{{ $group }}-management" value="{{  $group }}" data-member="{{ $name }}"
                                {{ AuthorizationHelper::roleHasPermissions($role, $permissions) ? 'checked' : '' }}>
                            <label for="{{ $group }}-management" class="title-label-select tree-header-text">
                                {{ $group }}</label>
                        </div>
                    </div>
                    <div class="col-md-6 tree-member role-{{ $group }}-management-checkbox">
                        @foreach ($permissions as $permission)
                            ...
                            <div class="form-check mt-4 mb-3 tree-item">
                                <input type="checkbox" name="permissions[]"
                                       id="check-permission-{{ $permission->id }}-edit"
                                       @class(['form-check-input permissions permission-member-input', $name . '-member-input', 'form-check-danger' => $danger])
                                       value="{{ $permission->name }}" data-head="{{ $name }}"
                                    @checked($role->hasPermissionTo($permission->name))>
                                <label for="check-permission-{{ $permission->id }}-edit"
    ...
    <div class="mt-3 d-flex justify-content-end">
        <a type="button" class="btn btn-secondary me-2" data-bs-dismiss="modal">{{ __('texts.close') }}</a>
        <button type="submit" class="btn btn-primary">{{ __('texts.update') }}</button>
    </div>
</form>
<script type="text/javascript">
    $('#check-all-permission-edit').change(function () {
